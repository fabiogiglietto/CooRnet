...
#' @importFrom httr oauth_endpoint oauth_app POST content
#' @importFrom jsonlite fromJSON
#'
get_ng_bearer <- function() {
  ...
  newsguard_endpoint <- oauth_endpoint(request="https://account.newsguardtech.com/account-auth/oauth/token",authorize = "https://account.newsguardtech.com/account-auth/oauth/token",access ="https://account.newsguardtech.com/account-auth/oauth/token")

  newsguard_app <- oauth_app("newsguard",
                             key = ng_key,
                             secret = ng_secret
  )
  ...
  req <- POST(newsguard_endpoint$access, encode = "form",
              body = list(
                client_id = newsguard_app$key,
                client_secret = newsguard_app$secret,
                redirect_uri = newsguard_app$redirect_uri,
                grant_type = "client_credentials"),
              authenticate(newsguard_app$key, newsguard_app$secret))
  req_json <- content(req,"text")
  bearer_token <- fromJSON(req_json)$access_token
