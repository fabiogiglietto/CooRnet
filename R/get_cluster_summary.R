...
get_cluster_summary <- function(output, machine_readable=FALSE){
  ...
  if (machine_readable == FALSE) {
    ...
    summary <- summary %>%
      ...
      dplyr::mutate(top.full_domain = gsub('.{2}$', '', paste0(na.omit(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df$cluster==cluster, "full_domain"])), desc(Freq))$Var1[1:10]), " (",
                                                               na.omit(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df$cluster==cluster, "full_domain"])), desc(Freq))$Freq[1:10]), "), ", collapse="")),
                    top.parent_domain = gsub('.{2}$', '', paste0(na.omit(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df$cluster==cluster, "parent_domain"])), desc(Freq))$Var1[1:10]), " (",
                                                                 na.omit(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df$cluster==cluster, "parent_domain"])), desc(Freq))$Freq[1:10]), "), ", collapse="")))
  } else {
    # add Gini score for full_domain and parent_domain
    ...
    summary <- summary %>%
      ...
      dplyr::mutate(gini.full_domain = ifelse(is.nan(gini.full_domain), 1, gini.full_domain),
                    gini.parent_domain = ifelse(is.nan(gini.parent_domain), 1, gini.parent_domain)) %>%
      dplyr::mutate(top.full_domain = list(paste0(na.omit(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df$cluster==cluster, "full_domain"])), desc(Freq))$Var1[1:10]), sep="")),
                    top.parent_domain = list(paste0(na.omit(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df$cluster==cluster, "parent_domain"])), desc(Freq))$Var1[1:10]), sep=""))) %>%
      dplyr::mutate(top.full_domain_freq = list(na.omit(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df$cluster==cluster, "full_domain"])), desc(Freq))$Freq[1:10])),
                    top.parent_domain_freq = list(na.omit(arrange(data.frame(table(ct_shares_marked.df[ct_shares_marked.df$cluster==cluster, "parent_domain"])), desc(Freq))$Freq[1:10])))
