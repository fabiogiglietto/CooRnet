...
#'
#' @examples
#' get_outputs_mongo(coord_shares_output=output)
#'
#' get_outputs_mongo(coord_shares_output=output, gdrive_folder_id="0BwwA4oUTeiV1TGRPeTVjaWRDY1E")
#'
...
get_outputs_mongo <- function(coord_shares_output,
  ...
  coord_shares_mdb <- coord_shares_output[[1]]

  if(is.null(coord_shares_mdb)) {
  ...
  coord_shares_output[[4]] <- coord_shares_output[[1]] # save connection info in the output list
  coord_shares_output[[1]] <- coord_shares_mdb$find() # import collection as a dataframe
  ...
  if (is.null(gdrive_folder_id)) {
    ...
    if (component_summary)
      ...
      component_summary <<- CooRnet::get_component_summary_mongo_ng(output = coord_shares_output)
    if (cluster_summary)
      # creates cluster_summary to the main environment
      cluster_summary <<- CooRnet::get_cluster_summary_mongo_ng(output = coord_shares_output)
    if (top_coord_urls)
